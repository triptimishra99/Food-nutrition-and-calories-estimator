...
if __name__ == '__main__':
    main_win = Tk()
    #app = Tk()
    img =Image.open('C:\\Users\\Tripti\OneDrive\Desktop\Project\\Food nutrition and calories estimator\\image\\bg.jpg')
    bg = ImageTk.PhotoImage(img)

    #app.geometry("650x450")

    # Add image
    label = Label(main_win, image=bg)
    label.place(x = 0,y = 0)
    ...
    Button(main_win,text='Quit',command=main_win.quit).grid(row=5,column=3,sticky=W,pady=4)
    Button(main_win,text=' Reduce Weight',command=Weight_Loss).grid(row=1,column=4,sticky=W,pady=4)
    Button(main_win,text='Gain Weight',command=Healthy).grid(row=3,column=4,sticky=W,pady=4)
    main_win.geometry("750x850")
    main_win.wm_title("DIET RECOMMENDATION BASED ON BMI")
